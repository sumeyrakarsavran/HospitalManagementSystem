import Kisiler.SaglikCalisanlari;
import java.sql.*;
import java.util.ArrayList;
import javax.swing.JOptionPane;

public class GirisSaglikCalisani extends javax.swing.JFrame {

    /**
     * Creates new form Giris
     */
    public GirisSaglikCalisani() {
        initComponents();
    }

    public ArrayList<SaglikCalisanlari> getSaglikCalisanlari() throws SQLException{
        Connection connection=null;
        DbHelper dbHelper =new DbHelper();
        Statement statement =null;
        ResultSet resultSet;
        ArrayList<SaglikCalisanlari> saglikCalisanlari=null;
        
        try{
            connection = dbHelper.getConnection();
            statement=connection.createStatement();
            resultSet=statement.executeQuery("select * from kisiler where Degree = 'Saglik Calisani' ");
            saglikCalisanlari=new ArrayList<SaglikCalisanlari>();
            
            while(resultSet.next()){
                saglikCalisanlari.add(new SaglikCalisanlari(
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Sağlık Çalışanı Giriş Sayfası");
    ...
    private void button1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_button1ActionPerformed
        int a =0;      
        try{
            ArrayList<SaglikCalisanlari> saglikCalisanlari = getSaglikCalisanlari();
            for(SaglikCalisanlari SaglikCalisani : saglikCalisanlari){       
                if(SaglikCalisani.getId().equals(tbxId.getText())){  
                    if(SaglikCalisani.getPassword().equals(String.valueOf(tbxPassword.getPassword()))){
                        a=1;
                        setVisible(false);
